...
const Index = () => {
  ...
  return (
    <div className="rounded-2xl bg-white border border-slate-200/80 p-6 shadow-xs space-y-6">
      ...
      {editingId !== null && (
        <div className="p-5 rounded-xl bg-orange-50/40 border border-orange-100/80 space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="rounded-xl bg-orange-600 px-4 py-1.5 text-xs font-bold text-white shadow-xs shadow-orange-600/20 hover:bg-orange-500 transition"
              >
      ...
      {addressBook.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {addressBook.map((addr) => (
            <div
              key={addr._id}
              className="p-4 rounded-xl border border-slate-200/80 bg-white hover:border-orange-200 flex flex-col justify-between space-y-3 transition"
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2.5">
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-orange-50 text-orange-600 border border-orange-100/80">
                    <IoHomeOutline size={15} />
                  </div>
                  <div>
                    <h5 className="font-heading text-xs font-bold text-slate-900">
                      {addr.name || "Delivery Address"}
                    </h5>
                    {addr.isDefault && (
                      <span className="text-[9px] font-bold uppercase text-orange-700 bg-orange-50 border border-orange-200 px-1.5 py-0.2 rounded">
                        Default
                      </span>
                    )}
                  </div>
        ...
        <div className="py-8 text-center text-slate-400">
          <p className="text-xs font-medium">No saved addresses yet</p>
        </div>
      )}
    </div>
  );
